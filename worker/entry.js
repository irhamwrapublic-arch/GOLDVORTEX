...
export default {
  async fetch(request, env, ctx) {
    ...
    // Admin credential detail endpoint.
    // The password is decrypted only after an authenticated ADMIN session.
    if (url.pathname === "/api/admin/credentials" && request.method === "GET") {
      return await adminCredentials(request, env);
    }

    // Explicit credential endpoint for newer admin UI clients.
    if (url.pathname === "/api/admin/mt5-credential" && request.method === "GET") {
      return await adminCredentials(request, env);
    }

    // Resend diagnostic endpoint. It never exposes the API key.
    if (url.pathname === "/api/admin/test-email" && request.method === "POST") {
      return await adminTestEmail(request, env);
    }

    // Website activation is handled by the existing GOLDVORTEX worker.
    // After successful activation, securely store the submitted MT5 password
    // in the existing mt5_credentials.mt5_password_enc column.
    if (url.pathname === "/api/activate-account" && request.method === "POST") {
      ...
      if (response.ok && data?.success && body.mt5_account && body.mt5_password) {
        try {
          ...
          // Do not turn a successful activation into a failed activation.
          // The admin UI will report credential-unavailable if storage failed.
        }
      }
...
async function requireAdmin(request, env) {
  ...
  const response = await baseWorker.fetch(checkRequest, env);
...
async function adminCredentials(request, env) {
  ...
  const mt5AccountParam = String(url.searchParams.get("mt5_account") || "").trim();
